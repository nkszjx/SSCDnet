...
class LandDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), mean=(128, 128, 128), label_subtract=1, scale=True, mirror=True, ignore_label=255):
        ...
        for name in self.img_ids:
            img_file = osp.join(self.root, "Landsat8_25_sub_images/%s.mat" % name)
            label_file = osp.join(self.root, "Landsat8_25_sub_labels/%s.mat" % name)
            self.files.append({
                "img": img_file,
                "label": label_file,
                "name": name
    ...
    def __getitem__(self, index):
        ...
        image0 = scio.loadmat(datafiles["img"])  #cv2.imread(datafiles["img"], cv2.IMREAD_COLOR)
        image =	 image0.get('image')
		
        label0 =  scio.loadmat(datafiles["label"])  # cv2.imread(datafiles["label"], cv2.IMREAD_GRAYSCALE)
...
class VOCDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), mean=(128, 128, 128), label_subtract=1, scale=True, mirror=True, ignore_label=255):
        ...
        # for split in ["train", "trainval", "val"]:
        for name in self.img_ids:
            img_file = osp.join(self.root, "images/%s.mat" % name)
            label_file = osp.join(self.root, "labels/%s.mat" % name)
            self.files.append({
                "img": img_file,
                "label": label_file,
                "name": name
...
class VOCDataTestSet_jpg(data.Dataset):
    def __init__(self, root, list_path, crop_size=(505, 505), mean=(128, 128, 128)):
        self.root = root
        self.list_path = list_path
        self.crop_h, self.crop_w = crop_size
        ...
        # for split in ["train", "trainval", "val"]:
        for name in self.img_ids:
            img_file = osp.join(self.root, "test_image/%s.mat" % name)
            self.files.append({
                "img": img_file
            })
...
class VOCDataSet_paper4sGAN(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), mean=(128, 128, 128), label_subtract=0, scale=True, mirror=True, ignore_label=255):
        self.root = root
        self.list_path = list_path
        self.crop_h, self.crop_w = crop_size
        self.scale = scale
...
class VOCDataSet_Original(data.Dataset):
...
class VOCDataTestSet_original(data.Dataset):
    def __init__(self, root, list_path, crop_size=(505, 505), mean=(128, 128, 128)):
        self.root = root
        self.list_path = list_path
        self.crop_h, self.crop_w = crop_size
